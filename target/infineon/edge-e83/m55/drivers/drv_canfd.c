                                  ...
                                  cy_stc_canfd_rx_buffer_t* rxBuffer)
{
    ...
    msg.ext_id = 0;
    msg.id_type = (rxBuffer->r0_f->xtd == CY_CANFD_XTD_EXTENDED_ID)
        ? CAN_ID_EXTENDED
        : CAN_ID_STANDARD;
    msg.frame_type = (rxBuffer->r0_f->rtr == CY_CANFD_RTR_REMOTE_FRAME)
        ? CAN_FRAME_REMOTE
        : CAN_FRAME_DATA;
    msg.data_len = ifx_canfd_dlc_to_len((uint8_t)rxBuffer->r1_f->dlc);
...
static int ifx_canfd_sendmsg(can_dev_t can_dev, const can_msg_t msg)
{
    ...
    uint32_t id;
    if (msg->id_type == CY_CANFD_XTD_EXTENDED_ID) {
...
static int ifx_canfd_recvmsg(can_dev_t can_dev, can_msg_t msg)
{
    ...
    if (can->rx_xtd == CY_CANFD_XTD_EXTENDED_ID) {
        msg->id_type = CAN_ID_EXTENDED;
        msg->ext_id = can->rx_id;
    } else {
        msg->id_type = CAN_ID_STANDARD;
        msg->std_id = can->rx_id & 0x7FF;
    }

    msg->frame_type = (can->rx_rtr == CY_CANFD_RTR_REMOTE_FRAME) ? CAN_FRAME_REMOTE
                                                                 : CAN_FRAME_DATA;
    copy_len = can->rx_len;
    if (copy_len > 8) {
        copy_len = 8;
    ...
    if (copy_len > 0) {
        memcpy(msg->data, can->rx_data, copy_len);
    }
